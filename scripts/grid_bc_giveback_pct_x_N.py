#!/usr/bin/env python3
"""Sweep giveback% × N for the BC_cross winner across all 8 quarters.

Current winner: BC_cross_GB_N1 with 0.75% giveback → +2,730% compound.
Test:
  giveback ∈ {0.5%, 0.75%, 1.0%}
  N (5m bars after trigger) ∈ {1, 2, 3}
  = 9 variants

Hist_x0 included as sanity check at the same matrix to see if a
better giveback% rescues histogram on the full set.
"""
...
BASE_PARAMS: dict = {
    ...
    # trail_stop_pct overridden per-variant; trail_stop_atr_multiple kept fixed
...
def _bc(condition: str, gb_pct: float, gb_N: int) -> dict:
    p = {
        ...
        "trail_stop_pct": gb_pct,
        "trail_stop_exit_on_signal": False,
        "trail_stop_giveback_window_bars": gb_N,
    }
...
# Reference: current shipping winner (signal-mode, 0.75% trail)
VARIANTS.append(("REF_BC_cross_signal_0.75", {
    "regime_trail_mode": "combined_bc", **B_PARAMS, **C_BASE,
...
# Previous best: cross + 0.75% + N=1
VARIANTS.append(("REF_BC_cross_GB075_N1", _bc("cross", 0.75, 1)))

# Sweep cross
for gb, n in product([0.5, 0.75, 1.0], [1, 2, 3]):
    VARIANTS.append((f"BC_cross_GB{int(gb*100)}_N{n}", _bc("cross", gb, n)))

# Sanity check: hist_x0 across same matrix
for gb, n in product([0.5, 0.75, 1.0], [1, 2, 3]):
    VARIANTS.append((f"BC_hist_GB{int(gb*100)}_N{n}", _bc("histogram", gb, n)))
...
def _run_one(args: tuple) -> dict:
    ...
    tmp = tempfile.mkdtemp(prefix="bc_gp_")
    try:
        cfg = Config({
            "backtest": {
                "name": f"bc_gp_{wk}_{tag}", "version": "bc-giveback-pct-x-N",
...
def main() -> None:
    ...
    out = REPO / "data" / "backtests" / "eth" / "combined_bc_grid" / "giveback_pct_x_N_8q.csv"
    pivot.to_csv(out)
    print(f"\nResults: {out}")
