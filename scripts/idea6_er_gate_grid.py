#!/usr/bin/env python3
"""ER-gate grid — tune (M, threshold) to find a config that preserves
2024_Q4 (held-flip critical) while still helping 2026_Q1 (held-flip hurts).

Research winner was M=48 N=0 ER≥0.15 (predicted +122pp on rejection PnL,
but per-quarter showed −62pp on 2024_Q4). Test progressively more
selective thresholds and shorter M values to soften the Q4 hit.

N is fixed at 0 (variant B was best across the research).
"""
...
# (M, threshold) — N pinned at 0. Plus REF.
VARIANTS = [
    ("REF_off", None, None),
    # M=48 (4h, the research winner timeframe) — sweep thresholds
    ("M48_T0.15", 48, 0.15),  # research winner
    ("M48_T0.20", 48, 0.20),
    ("M48_T0.25", 48, 0.25),
    ("M48_T0.30", 48, 0.30),
    ("M48_T0.35", 48, 0.35),
    # M=24 (2h) — shorter window, may detect transient trends only
    ("M24_T0.30", 24, 0.30),
    ("M24_T0.40", 24, 0.40),
    ("M24_T0.50", 24, 0.50),
    # M=12 (1h)
    ("M12_T0.40", 12, 0.40),
    ("M12_T0.50", 12, 0.50),
    # M=72 / 96 (longer, may smooth noise but lose responsiveness)
    ("M72_T0.20", 72, 0.20),
    ("M72_T0.25", 72, 0.25),
    ("M96_T0.20", 96, 0.20),
    ("M36_T0.25", 36, 0.25),
...
def _run_one(args: tuple) -> dict:
    ...
    tmp = tempfile.mkdtemp(prefix="erg_")
    try:
        ...
        cfg = Config({
            "backtest": {
                "name": f"erg_{wk}_{tag}", "version": "er-gate-grid",
    ...
    print(
        f"  {wk} {tag:<14} ret={ret:+8.2f}% WR={tm['wr_pct']:5.1f}% "
        f"er={tm['er_exits']:>3d} safe={tm['safety_exits']:>2d} "
        f"rej_bars={tm['rejected_hold_bars']:>3d} {elapsed:.1f}s",
        flush=True,
    )
    return {
        "window": wk, "tag": tag, "return_pct": ret, "wr_pct": tm["wr_pct"],
        "er_exits": tm["er_exits"], "safety_exits": tm["safety_exits"],
...
def main() -> None:
    ...
    out_dir = REPO / "data" / "backtests" / "eth" / "idea6_er_gate_grid"
    out_dir.mkdir(parents=True, exist_ok=True)
    pivot.to_csv(out_dir / "compound_per_variant.csv")
    df.to_csv(out_dir / "raw_runs.csv", index=False)

    # Profile + key-quarter focus
    print("\n" + "=" * 130)
    print(f"{'Tag':<14} {'avgEr/q':>9} {'avgSafe/q':>10} {'2024_Q4':>10} {'2026_Q1':>10} "
          f"{'avgWR%':>8} {'avgSharpe':>11}")
    print("-" * 130)
    for tag in pivot.index:
        sub = df[df["tag"] == tag]
        q4 = sub[sub["window"] == "2024_Q4"]["return_pct"].iloc[0]
        q26q1 = sub[sub["window"] == "2026_Q1"]["return_pct"].iloc[0]
        print(f"{tag:<14} {sub['er_exits'].mean():>9.1f} "
              f"{sub['safety_exits'].mean():>10.1f} "
              f"{q4:>+10.2f} {q26q1:>+10.2f} "
