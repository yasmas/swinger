#!/usr/bin/env python3
"""HH/LL gate — run 3 winners on the remaining 4 quarters, merge full 8q picture."""
...
# Remaining 4 quarters not in the initial sweep
...
VARIANTS = [
    ...
    ("ER_HHLL_K48_X0.30", {**ER_PARAMS,
                           "flip_hhll_gate_enabled": True,
                           "flip_hhll_gate_period": 48,
                           "flip_hhll_gate_min_mag_pct": 0.30}),
    ("ER_HHLL_K36_X0.20", {**ER_PARAMS,
                           "flip_hhll_gate_enabled": True,
                           "flip_hhll_gate_period": 36,
                           "flip_hhll_gate_min_mag_pct": 0.20}),
    ("ER_HHLL_K60_X0.30", {**ER_PARAMS,
                           "flip_hhll_gate_enabled": True,
                           "flip_hhll_gate_period": 60,
                           "flip_hhll_gate_min_mag_pct": 0.30}),
]
...
def _run_one(args: tuple) -> dict:
    ...
    tmp = tempfile.mkdtemp(prefix="hhll_full_")
    try:
        params = {**SHIP_PARAMS, **overrides}
        cfg = Config({
            "backtest": {
                "name": f"hhll_full_{wk}_{tag}", "version": "hhll-full",
...
def main() -> None:
    # Run remaining 4 quarters for the 5 variants
    ...
    # Load initial sweep raw runs and pull out the variants we care about
    init_csv = REPO / "data" / "backtests" / "eth" / "idea7_hhll_initial_sweep" / "raw_runs.csv"
    init_df = pd.read_csv(init_csv)
    keep_tags = {tag for tag, _ in VARIANTS}
    init_subset = init_df[init_df["tag"].isin(keep_tags)]

    # Concat and pivot for 8-quarter view
    ...
    print("=" * 130)
    print(f"{'Tag':<22} {'avgPP/q':>9} {'avgER/q':>9} {'avgHH/q':>9} "
