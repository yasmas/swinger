#!/usr/bin/env python3
"""Run candidate LazySwing profit-protection regime exits."""
...
RUNS = [
    (
        "relaxed_stretch_adx25_no_reentry",
        {
            **REGIME_COMMON,
...
def exit_win_rate(trade_log: pd.DataFrame) -> tuple[float, int, int, int]:
    ...
    for _, row in trade_log.iterrows():
        ...
        if pnl > 0:
            ...
            losses += 1
    exits = wins + losses
    return (wins / exits * 100.0 if exits else float("nan")), wins, losses, exits
...
def build_config(period: dict, tag: str, params_extra: dict) -> Config:
    ...
    return Config(
        {
            "backtest": {
                "name": f"LazySwing_takeprofit_{period['period']}_{tag}",
                "version": "takeprofit-options",
...
def run_one(period: dict, tag: str, params_extra: dict, output_root: Path) -> dict:
    ...
    wr, wins, losses, exits = exit_win_rate(trade_log)
    ...
    print(
        f"{period['period']:<8} {tag:<28} ret={row['total_return_pct']:+8.2f}% "
...
def main() -> None:
    output_root = REPO / "reports" / "lazyswing-takeprofit-relaxed-adx25-selected"
    rows = [
