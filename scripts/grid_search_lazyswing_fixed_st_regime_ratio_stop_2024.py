#!/usr/bin/env python3
"""Grid search regime-driven ratio/stop control with a fixed ST for ETH 2024."""
...
# Reuse the same slow regime classifier that worked best for adaptive ST, but
# keep the ST itself fixed and let only the flip-vol ratio gate consume it.
REGIME_PARAMS = {
    "adaptive_st_enabled": False,
...
GRID_PRESETS = {
    "narrow": {
        "modes": ["step", "linear"],
        "low_ratio": [0.7, 0.75],
        "high_ratio": [0.8, 0.9, 1.0],
        "low_stop": [1.0],
        "high_stop": [1.5, 2.0, 2.5],
    },
    "focused": {
        "modes": ["step", "linear"],
        "low_ratio": [0.75, 0.8],
        "high_ratio": [0.9, 0.95, 1.0],
        "low_stop": [1.0],
        "high_stop": [1.5, 2.0, 2.5],
    },
    "refined": {
        "modes": ["step", "linear"],
        "low_ratio": [0.75, 0.78, 0.8],
        "high_ratio": [0.9, 0.925, 0.95],
        "low_stop": [1.0],
        "high_stop": [1.5, 2.0],
    },
    "broad": {
...
def build_runs(stage: str) -> list[tuple[str, dict, dict]]:
    ...
    for mode in preset["modes"]:
        for low_ratio in preset["low_ratio"]:
            for high_ratio in preset["high_ratio"]:
                if high_ratio < low_ratio:
                    continue
                for low_stop in preset["low_stop"]:
                    for high_stop in preset["high_stop"]:
                        ...
                        meta = {
                            "family": "regime_dynamic_fixed_st",
                            "mode": mode,
...
def main() -> None:
    ...
    output_root = REPO / "reports" / "fixed-st-regime-ratio-stop-eth" / args.label
    output_root.mkdir(parents=True, exist_ok=True)

    runs = build_runs(args.stage)
    print(
        f"LazySwing fixed-ST regime ratio/stop grid: {args.label}  "
    ...
    for tag, params, meta in runs:
        ...
        cfg = Config(
            {
                "backtest": {
                    "name": f"LazySwing_fixed_st_regime_ratio_stop_{args.label}_{tag}",
                    "version": "fixed-st-regime-ratio-stop",
