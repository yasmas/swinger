#!/usr/bin/env python3
"""HH/LL gate initial sweep — 4 quarters (2 good, 2 bad).

PP4 stays on throughout. Tests:
  - REF (PP4 only, no ER, no HH/LL)
  - PP4 + ER48_T0.32 (= v5 ship)
  - PP4 + HH/LL(K, X) for each (K, X) combo
  - PP4 + ER48_T0.32 + HH/LL(K, X) for each combo

K ∈ {36, 48, 60}, X ∈ {0.15, 0.20, 0.30}
= 3 + 9 + 9 = 21 variants × 4 quarters = 84 runs.
"""
...
# Initial sweep on 2 good + 2 bad
WINDOWS = {
    "2025_Q2": {"start": "2025-04-01", "end": "2025-07-01"},  # GOOD
    "2026_Q1": {"start": "2026-01-01", "end": "2026-04-01"},  # GOOD
    "2024_Q4": {"start": "2024-10-01", "end": "2025-01-01"},  # BAD
    "2024_Q2": {"start": "2024-04-01", "end": "2024-07-01"},  # BAD
}
...
SHIP_PARAMS: dict = {
    ...
    # PP4 always on (per user direction)
    "flip_protect_min_gain_pct": 4.0,
}

K_VALS = [36, 48, 60]
X_VALS = [0.15, 0.20, 0.30]
...
def _build_variants() -> list[tuple[str, dict]]:
    ...
    variants.append(("PP4_ER48_T0.32", {
        "flip_er_gate_period": 48,
        "flip_er_gate_exclude_bars": 0,
        "flip_er_gate_threshold": 0.32,
    }))
    # HH/LL alone (PP4 only + HH/LL)
    for K in K_VALS:
        for X in X_VALS:
            tag = f"HHLL_K{K}_X{X:.2f}"
            variants.append((tag, {
                "flip_hhll_gate_enabled": True,
                "flip_hhll_gate_period": K,
                "flip_hhll_gate_min_mag_pct": X,
            }))
    # HH/LL + ER (PP4 + ER + HH/LL)
    for K in K_VALS:
        for X in X_VALS:
            tag = f"ER_HHLL_K{K}_X{X:.2f}"
            variants.append((tag, {
                "flip_er_gate_period": 48,
                "flip_er_gate_exclude_bars": 0,
                "flip_er_gate_threshold": 0.32,
...
def _trade_metrics(trade_log: pd.DataFrame) -> dict:
    ...
    for _, row in trade_log.iterrows():
        ...
        if row["action"] in ("SELL", "COVER"):
            ...
            elif ex == "st_flip_hhll_gate": hh_n += 1
            elif ex == "st_flip_ratio_safety": safety_n += 1
    total = wins + losses
    return {
        "wr_pct": (wins / total * 100.0) if total else float("nan"),
        "pp_exits": pp_n, "er_exits": er_n, "hh_exits": hh_n, "safety_exits": safety_n,
    }
...
def _run_one(args: tuple) -> dict:
    ...
    tmp = tempfile.mkdtemp(prefix="hhll_")
    try:
        params = {**SHIP_PARAMS, **overrides}
        cfg = Config({
            "backtest": {
                "name": f"hhll_{wk}_{tag}", "version": "hhll-initial",
    ...
    print(
        ...
        f"pp={tm['pp_exits']:>2d} er={tm['er_exits']:>3d} hh={tm['hh_exits']:>3d} "
        f"safe={tm['safety_exits']:>2d}",
        flush=True,
    )
    return {
        "window": wk, "tag": tag, "return_pct": ret, "wr_pct": tm["wr_pct"],
        "pp_exits": tm["pp_exits"], "er_exits": tm["er_exits"], "hh_exits": tm["hh_exits"],
...
def main() -> None:
    ...
    # Score = sum across 4 quarters (rough proxy; not full compound)
    ...
    print("=" * 130)
    print(f"{'Tag':<22} {'2025_Q2':>10} {'2026_Q1':>10} {'2024_Q4':>10} {'2024_Q2':>10} "
          f"{'Sum%':>9} {'Mean%':>9} {'Min%':>9}")
    print("-" * 130)
    for tag, r in pivot.iterrows():
        print(f"{tag:<22} {r['2025_Q2']:>+10.2f} {r['2026_Q1']:>+10.2f} "
              f"{r['2024_Q4']:>+10.2f} {r['2024_Q2']:>+10.2f} "
              f"{r['sum_pct']:>+9.2f} {r['mean_pct']:>+9.2f} {r['min_pct']:>+9.2f}")

    out_dir = REPO / "data" / "backtests" / "eth" / "idea7_hhll_initial_sweep"
