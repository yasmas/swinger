#!/usr/bin/env python3
"""Idea #2 refinement: replace scalar ADX-decline test with linear-regression
slope on the last W 30m bars.

Sweep:
  slope_window ∈ {6, 8, 12} (3h, 4h, 6h)
  slope_drop_pct ∈ {3.5%, 5%, 7.5%}
  = 9 BC variants

Reference: BC_m8-21-9_adx14_lb12 (current ship, scalar mode).
"""
...
def _bc(slope_window: int = 0, drop_pct: float = 3.5,
        scalar_lookback: int = 12) -> dict:
    p = {
        "regime_trail_mode": "combined_bc",
        # B (adx_exhaustion):
        "regime_exhaustion_adx_lookback": scalar_lookback,
        "regime_exhaustion_adx_drop_pct": drop_pct,
    ...
    if slope_window > 0:
        p["regime_exhaustion_adx_slope_window"] = slope_window
    return p


VARIANTS: list[tuple[str, dict]] = [
    ("REF_scalar_lb12_drop3.5", _bc(scalar_lookback=12, drop_pct=3.5)),
]
for sw, drop in product([6, 8, 12], [3.5, 5.0, 7.5]):
    VARIANTS.append((f"slope_W{sw}_drop{drop}", _bc(slope_window=sw, drop_pct=drop)))
...
def _run_one(args: tuple) -> dict:
    ...
    tmp = tempfile.mkdtemp(prefix="bc_slope_")
    try:
        cfg = Config({
            "backtest": {
                "name": f"bc_slope_{wk}_{tag}", "version": "bc-adx-slope",
    ...
    print(
        f"  {wk} ({win['favored']}) {tag:<28} ret={ret:+8.2f}% "
...
def main() -> None:
    ...
    for tag, r in pivot.iterrows():
        ...
        print(f"{tag:<28} " + "".join(f"{x:>+8.2f}" for x in rets)
              + f" {r.compound_pct:>+10.2f} {r.mean_pct:>+8.2f} {r.min_pct:>+8.2f}")

    out = REPO / "data" / "backtests" / "eth" / "combined_bc_grid" / "adx_slope_8q.csv"
    pivot.to_csv(out)
    print(f"\nResults: {out}")
