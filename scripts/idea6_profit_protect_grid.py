#!/usr/bin/env python3
"""Profit-protect grid — flip_protect_min_gain_pct ∈ {2, 3, 4, 5, off}."""
...
VARIANTS = [
    ...
    ("protect_X4", 4.0),
    ("protect_X5", 5.0),
    # bonus: very tight & very loose for sensitivity
    ("protect_X1.5", 1.5),
    ("protect_X7", 7.0),
]
...
def _run_one(args: tuple) -> dict:
    wk, tag, threshold = args
    win = WINDOWS[wk]
    slice_file = str(SLICE_DIR / f"{wk}.csv")
    tmp = tempfile.mkdtemp(prefix="ppg_")
    try:
        params = {**SHIP_PARAMS, "flip_protect_min_gain_pct": threshold}
        cfg = Config({
            "backtest": {
                "name": f"ppg_{wk}_{tag}", "version": "profit-protect-grid",
    ...
    print(
        ...
        f"prot={tm['protect_exits']:>2d} safe={tm['safety_exits']:>2d} "
        f"rej_bars={tm['rejected_hold_bars']:>3d} {elapsed:.1f}s",
        flush=True,
    )
    return {
        "window": wk, "tag": tag, "return_pct": ret, "wr_pct": tm["wr_pct"],
        "protect_exits": tm["protect_exits"], "safety_exits": tm["safety_exits"],
...
def main() -> None:
    tasks = [(wk, tag, t) for wk in WINDOWS for tag, t in VARIANTS]
    ...
    out_dir = REPO / "data" / "backtests" / "eth" / "idea6_profit_protect_grid"
    out_dir.mkdir(parents=True, exist_ok=True)
    pivot.to_csv(out_dir / "compound_per_variant.csv")
    df.to_csv(out_dir / "raw_runs.csv", index=False)

    print("\n" + "=" * 130)
    print("Profile per variant (averaged across 8 quarters)")
    print("=" * 130)
    print(f"{'Tag':<14} {'avgProt/q':>10} {'avgSafe/q':>10} {'avgRejBars/q':>14} "
          f"{'avgWR%':>9} {'avgSharpe':>11} {'avgMaxDD%':>11}")
    print("-" * 130)
    for tag in pivot.index:
        sub = df[df["tag"] == tag]
        print(f"{tag:<14} {sub['protect_exits'].mean():>10.1f} "
