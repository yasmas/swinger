...
- **Do not attempt sub-hourly ST re-evaluation.** The hourly close aggregation is a core part of the strategy — it filters intra-hour noise. Comparing 5m closes against hourly bands mid-hour produces false flips (price temporarily crosses band but recovers by hourly close). Tested exhaustively: unfiltered 30m/15m/5m checks, filtered with ADX/Volume/CMF, exit-only mode — all worse than baseline. Even the best filter combo (ADX>25 + Vol>1.5σ) lost ~half the return. See Experiment 3 in context-lazyswing.md.
- **Do not attempt pre-flip early exit based on proximity to ST band.** The ST band acts as support/resistance — near-band bars that DON'T flip tend to bounce strongly (0.34–0.43% avg next-hour move in trend direction). This makes the real false-positive cost 3–4× higher than the naive transaction-cost assumption (0.10%). All signal variants tested (simple dist_atr, peak-then-retreat, progressive 2h/3h/4h monotonic approach, approach-rate filters) showed negative EV. Implementation and full backtest confirmed: dist_atr<0.8 degraded total return 14× (209M% → 14.9M%). **Always measure actual H→H+1 price-move cost for false positives; never assume FP cost = transaction cost alone.** See Experiment 4 in context-lazyswing.md.
- **Any signal with n < 100 instances in the dev set is noise, not a signal.** Fast-ST flip + good PnL showed 46% precision (EV +0.15%) on dev (n=28) but collapsed to 20% precision (EV -0.18%) on the test set. With n=28, the 95% confidence interval spans ±18pp — the signal is indistinguishable from chance.
- **When simulating early-exit EV, always compute realistic FP cost from data, not just transaction costs.** The formula `EV = P(flip)*benefit - P(no_flip)*0.10` is wrong for price-proximity signals. Always measure the actual average price move H→H+1 for bars matching the signal but NOT followed by a flip. For ST strategies, near-band FP bars have outsized bounces, making the true FP cost ~0.3–0.4%. The breakeven precision is ~26%, achievable only at dist<0.3 (fires 2% of bars, barely positive EV).
- **"True positives" in EV analysis can still hurt in a backtest when the flip bar is HIGHER than the exit bar.** The EV analysis correctly identified precision (17–27%) but missed a hidden cost: even when the ST DOES flip at T+1, the price at T+1 (hourly close) may be HIGHER than the exit price at T (after a brief intra-hour bounce). In 5 of 7 actual TP cases, the flip bar closed higher than the DMI exit bar — meaning we exited the long for less money than if we'd held. This is distinct from the FP bounce problem: even confirmed flips can have price recovery BETWEEN the exit and the flip bar. EV analysis using hourly OHLC data cannot detect this intra-hour path effect. **Conclusion: any exit-before-hourly-flip strategy faces this intra-hour recovery risk, which is essentially unmodelable without sub-hourly data at the bar level.** See Experiment 5 in context-lazyswing.md.
- **ADX/DMI, RSI divergence, OBV divergence, Stochastic, CCI, Elder Ray, Parabolic SAR, and EMA crossovers all have negative EV as early-exit triggers.** Absolute indicator levels (e.g., Stochastic > 75) fire during strong trends (34% of all bars), giving precision well below baseline. Direction-change signals (DI+ crossing below DI-, RSI crossing 50) are better (3x lift, 9% precision) but still negative EV due to the intra-hour recovery problem. Combined DMI + pull + profit + dist filters showed +0.13% EV in analysis but -13% in backtest. No pre-flip early exit approach has worked across 5 experiments. **This line of investigation should be considered exhausted.**
